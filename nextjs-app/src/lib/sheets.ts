import type { UsageData, StudentData, DivisionData, UserProfile } from '@/types';
...
async function apsFetch<T>(endpoint: string, extra: Record<string, string> = {}): Promise<T> {
  if (!SCRIPT_URL) throw new Error('APPS_SCRIPT_URL environment variable is not set');
  if (!API_KEY) throw new Error('APPS_SCRIPT_API_KEY environment variable is not set');
  ...
  const res = await fetch(url.toString(), {
    // Apps Script URLs follow redirects — next/fetch handles this automatically
    redirect: 'follow',
